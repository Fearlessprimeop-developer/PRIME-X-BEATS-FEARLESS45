...
import asyncio, logging, time, inspect, secrets, os
...
class PrimeBeats:
    ...
    async def _restart_current(self,chat_id,position:float|None=None):
        ...
        if change is not None:
            # change_stream is the important part: it swaps the media source
            # inside the SAME Telegram group call.
    ...
    def _register(self):
        ...
        async def start(_,m):
            ...
            # Rich FEARLESS 45 private welcome card. Put the supplied image at
            # assets/fearless45.png in the deployed project, or override with
            # START_IMAGE_PATH / START_IMAGE_URL.
            is_private=str(getattr(getattr(m,"chat",None),"type","")).lower().endswith("private")
            image_path=__import__("os").environ.get(
                "START_IMAGE_PATH",
                __import__("os").path.join(__import__("os").path.dirname(__file__),"assets","fearless45.png")
            )
            image_url=__import__("os").environ.get("START_IMAGE_URL","").strip()
            if is_private:
                try:
                    photo=image_url or (image_path if __import__("os").path.isfile(image_path) else None)
                    if photo:
                        await self.bot.send_photo(m.chat.id,photo,caption=caption,reply_markup=home_keyboard(self.cfg))
                        return
                except Exception:
                    log.exception("PRIVATE START IMAGE SEND FAILED")
            await m.reply_text(caption,reply_markup=home_keyboard(self.cfg))
        ...
        async def yourdaddy(_,m):
            await m.reply_text(
                ...
                "</blockquote>\n\n"
                "<i>PRIME × BEATS • Built different.</i>",
                reply_markup=links(self.cfg)
            )
