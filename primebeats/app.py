...
class PrimeBeats:
    ...
    def _register(self):
        ...
        async def start(_,m):
            # Private /start intentionally uses the clean text welcome layout
            # shown in the reference: no start photo is attached.
        ...
        async def help_cmd(_,m):
            ...
            image=os.environ.get("HELP_IMAGE_PATH",os.path.join(root,"assets","fearless_help.png"))
            text=help_text(self.cfg.bot_name)
            if os.path.isfile(image):
                try:
                    await self.bot.send_photo(m.chat.id,image,caption=text,reply_markup=home_keyboard(self.cfg))
                    return
                except Exception:
                    log.exception("HELP IMAGE SEND FAILED")
            await m.reply_text(text,reply_markup=home_keyboard(self.cfg))
        ...
        async def yourdaddy(_,m):
            root=os.path.dirname(__file__)
            image=os.environ.get("DADDY_IMAGE_PATH",os.path.join(root,"assets","fearless_daddy.jpg"))
            caption="<b>My daddy................................</b>"
            if os.path.isfile(image):
                try:
                    await self.bot.send_photo(m.chat.id,image,caption=caption)
                    return
                except Exception:
                    log.exception("YOURDADDY IMAGE SEND FAILED")
            await m.reply_text(caption)
        ...
        async def callbacks(_,q):
            ...
            if data=="searchclose":
                ...
                await q.message.edit_text(
                    ...
                    "┃ ⚡ <b>FEARLESS 45</b>\n"
                    "┃ 👤 @Prime_Fearless_45\n"
                    "┃ 🆔 <code>7915543522</code>\n"
                    "<b>╰━━━━━━━━━━━━━━━━━━━━╯</b>\n\n"
                    "<blockquote>"
                    "⚡ No need to compare. The engine does the talk.\n"
                    "🖤 Quiet mind. Loud results.\n"
                    "👑 Owner only. No excuses.\n"
                    "🔥 Fearless 45 — Stay under the brand."
                    "</blockquote>",
                    reply_markup=home_keyboard(self.cfg)
                )
                await q.answer("Owner profile ⚡")
