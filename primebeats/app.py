...
class PrimeBeats:
    ...
    async def _restart_current(self,chat_id,position:float|None=None):
        """Apply FX/seek to the current track without leaving or recreating the VC."""
        p=self.store.get(chat_id)
        ...
        # Reuse the already-working source. Re-resolving on every FX click can
        # replace a good VC stream with a fresh provider connection and is the
        # reason effect changes could appear to close the call.
        ...
        if change is not None:
            # Swap the media inside the SAME Telegram group call. Some PyTgCalls
            # builds report ChangedStream but leave the stream paused; explicitly
            # resume it so an FX click never leaves a silent VC.
            ...
            # Older builds may not expose change_stream. play() replaces the
            # stream without explicitly calling leave_call, so the existing VC
            # remains available and the assistant can continue in it.
    ...
    def _register(self):
        ...
        async def send_help_message(chat_id):
            ...
            if os.path.isfile(image):
                try:
                    return await self.bot.send_photo(chat_id, image, caption=text, reply_markup=home_keyboard(self.cfg))
                except Exception:
            ...
            if os.path.isfile(image):
                try:
                    return await self.bot.send_photo(
                        chat_id, photo=image, caption=caption, reply_markup=links(self.cfg)
                    )
                except Exception:
        ...
        async def start(_,m):
            ...
            if is_private:
                ...
                if os.path.isfile(video):
                    try:
                        await self.bot.send_video(m.chat.id, video, caption="🎵 <b>FEARLESS × MUSIC</b>\n⚡ Music • Stream • Enjoy", supports_streaming=True)
                    except Exception:
        ...
        async def ping(_,m):
            ...
            try:
                ...
                if os.path.isfile(image):
                    await self.bot.send_photo(m.chat.id,image,caption=caption)
                else:
