...
async def resolve(link_or_query: str, requested_by: str = "Unknown",
    ...
    # FAST PATH: let PyTgCalls read Yuki's HTTP stream directly. This avoids
    # waiting for the complete MP3/MP4 download before playback can begin.
    # If Yuki is unavailable, fall back to the existing local downloader.
    metadata_duration = _seconds(info.get("duration"))
    media_path = None
    if API_KEY and API_KEY != "YOUR_API_KEY":
        req_type = "video" if video else "audio"
        ...
        try:
            ...
            async with session.get(direct_url, allow_redirects=True) as resp:
                if resp.status == 200:
                    # For /vplay, never accept an audio-only response as a video source.
                    content_type = (resp.headers.get("Content-Type") or "").lower()
                    if video and content_type.startswith("audio/"):
                        print(f"[Yuki API] fast video endpoint returned audio ({content_type}); using video downloader")
                    else:
                        # Do not consume the body here: PyTgCalls will stream it.
                        media_path = str(resp.url)
                        print(f"[Yuki API] FAST STREAM READY: {media_path.split('?')[0]} type={content_type or 'unknown'}")
                else:
                    print(f"[Yuki API] fast stream HTTP {resp.status}; using downloader")
        except Exception as exc:
            print(f"[Yuki API] fast stream error: {exc}; using downloader")
