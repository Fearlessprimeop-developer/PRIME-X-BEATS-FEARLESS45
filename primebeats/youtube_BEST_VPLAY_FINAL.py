...
async def resolve(link_or_query: str, requested_by: str = "Unknown",
    ...
    # Audio can use Yuki's direct HTTP stream for near-instant playback.
    # Video is deliberately downloaded through Yuki first and ffprobe-verified
    # before PyTgCalls receives it. This guarantees /vplay gets a REAL video
    # stream with its audio instead of accepting an audio-only provider URL.
    metadata_duration = _seconds(info.get("duration"))
    media_path = None
    if video:
        media_path = await download_media(info["id"], is_video=True)
        print(f"[Yuki API] VERIFIED VIDEO READY: {media_path}")
    elif API_KEY and API_KEY != "YOUR_API_KEY":
        direct_url = (
            f"{API_URL}/stream/{info['id']}?key={API_KEY}"
            f"&type=audio&quality=360"
        )
        try:
            session = await get_session()
            async with session.get(direct_url, allow_redirects=True) as resp:
                if resp.status == 200:
                    media_path = str(resp.url)
                    print(f"[Yuki API] FAST AUDIO READY: {media_path.split('?')[0]}")
                else:
                    print(f"[Yuki API] fast audio HTTP {resp.status}; using downloader")
        except Exception as exc:
            print(f"[Yuki API] fast audio error: {exc}; using downloader")
