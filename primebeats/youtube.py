...
def _ytdlp_download_sync(video_id: str, is_video: bool, out_dir: str,
    ...
    if is_video:
        base_opts.update({
            # Select the highest-quality video stream at or below 480p, with
            # the best available audio. If YouTube has no <=480p video stream,
            # fall back to the best combined format. Telegram/PyTgCalls then
            # outputs through its 480p profile.
            # Best available source up to 480p. Prefer a 60-fps 480p source when
            # YouTube exposes one, but never exceed Telegram/PyTgCalls' 480p path.
            "format": (
...
async def download_media(video_id: str, is_video: bool = False) -> str:
    ...
    # Telegram/PyTgCalls video output is configured to SD 480p for this bot.
