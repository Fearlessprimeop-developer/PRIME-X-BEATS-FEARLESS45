...
API_URL = (
    os.environ.get("MEOW_API_URL")
    or os.environ.get("YUKI_API_URL")
    or "https://music.yukiapi.site"
).rstrip("/")

# Accept the common Yuki/Meow variable names so the Yuki provider is not
# accidentally skipped on Render because the key was stored under another
# expected name. Yuki is ALWAYS attempted before yt-dlp.
API_KEY = (
    os.environ.get("MEOW_API_KEY")
    or os.environ.get("YUKI_API_KEY")
    or os.environ.get("MEOWAPI_KEY")
    or "YOUR_API_KEY"
) ## Get this API KEY from @MeowApiRobot
...
def _ytdlp_download_sync(video_id: str, is_video: bool, out_dir: str,
    ...
    """Download a local media file with yt-dlp. /vplay uses this as primary."""
    import yt_dlp
    ...
    if is_video:
        base_opts.update({
            # /vplay PRIMARY selector requested by the user:
            # highest-quality video stream at or below 480p + best audio.
            # This intentionally uses yt-dlp first for /vplay only.
            "format": "bestvideo[height<=480]+bestaudio",
            "merge_output_format": "mp4",
            "format_sort": ["res:480", "fps", "vcodec:h264", "acodec:aac"],
        })
    else:
        base_opts.update({
...
async def download_media(video_id: str, is_video: bool = False) -> str:
    """Download media; /vplay uses yt-dlp as primary, audio keeps Yuki-first."""
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)

    # IMPORTANT: /vplay is the only video playback path. For video, yt-dlp is
    # deliberately PRIMARY with the exact requested selector:
    # bestvideo[height<=480] + bestaudio. Yuki is not tried before it.
    if is_video:
        try:
            path = await _download_with_ytdlp(video_id, True)
            if await _probe_media(path, True):
                print(f"[yt-dlp PRIMARY /vplay] 480p video ready: {path}")
                return path
            raise RuntimeError("yt-dlp produced a video without a valid video stream")
        except Exception as exc:
            print(f"[yt-dlp PRIMARY /vplay] failed: {type(exc).__name__}: {exc}")
            # Only after the primary yt-dlp attempt fails do we try Yuki.
            # Audio playback remains Yuki-first below.
...
async def resolve(link_or_query: str, requested_by: str = "Unknown",
    ...
    # Use a verified LOCAL media file for both audio and video. This avoids
    # expired/redirecting provider URLs becoming silent inside the VC, and it
    # gives FX/seek a stable source that can be reused with change_stream.
    metadata_duration = _seconds(info.get("duration"))
    media_path = await download_media(info["id"], is_video=bool(video))
    print(f"[Yuki/yt-dlp] VERIFIED {'VIDEO' if video else 'AUDIO'} READY: {media_path}")
