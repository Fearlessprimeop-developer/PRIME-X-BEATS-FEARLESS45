...
async def _search_many(query: str, limit: int = 10) -> list[dict]:
    # Fast metadata provider first. Normal /search never starts yt-dlp.
    try:
...
def _cookie_file() -> str | None:
    ...
    # Render Secret Files are read-only. yt-dlp receives only a writable /tmp copy.
    ...
    try:
        ...
        needs_copy = (
            ...
            or os.path.getmtime(writable) < source_mtime
        )
        if needs_copy:
            tmp_copy = writable + ".tmp"
            import shutil as _shutil
            _shutil.copyfile(source, tmp_copy)
            os.chmod(tmp_copy, 0o600)
            os.replace(tmp_copy, writable)
        else:
...
def _ytdlp_download_sync(video_id: str, is_video: bool, out_dir: str,
    ...
    # Never pass Render's read-only /etc/secrets file to yt-dlp.
    if cookie_file and os.path.abspath(cookie_file).startswith("/tmp/"):
...
async def download_media(video_id: str, is_video: bool = False) -> str:
    ...
    qualities = (480, 360) if is_video else (360,)
    session = await get_session()
    last_error = None
...
async def _yuki_fast_stream(video_id: str, is_video: bool) -> str | None:
    """Validate and return a Yuki stream URL without downloading the media."""
    if not API_KEY or API_KEY == "YOUR_API_KEY":
        print("[Yuki API] API key unavailable; primary stream disabled")
        return None

    req_type = "video" if is_video else "audio"
    quality = 480 if is_video else 360
    safe_key = urllib.parse.quote(API_KEY, safe="")
    base = API_URL.rstrip("/")
    candidates = [
        f"{base}/stream/{video_id}?key={safe_key}&type={req_type}&quality={quality}",
        f"{base}/stream/{video_id}?key={safe_key}&type={req_type}",
    ]
    ...
    headers = {
        "Range": "bytes=0-1",
        "Accept": "*/*",
        "User-Agent": "PRIME-BEATS/1.0",
    }

    for attempt, url in enumerate(candidates, 1):
        try:
            async with session.get(
                ...
                if resp.status in (301, 302, 303, 307, 308) and location:
                    ...
                    print(
                        f"[Yuki API] FAST STREAM READY "
                        f"(redirect, attempt={attempt}, type={req_type})"
                    )
                    return final_url

                content_type = (resp.headers.get("Content-Type") or "").lower()
                if resp.status in (200, 206):
                    # Yuki may use application/octet-stream for media. Reject only
                    # obvious JSON/HTML/text responses.
                    if not any(
                        bad in content_type
                        for bad in ("application/json", "text/html", "text/plain")
                    ):
                        print(
                            f"[Yuki API] FAST STREAM READY "
                            f"(direct, attempt={attempt}, type={req_type})"
                        )
                        return url

                body = await resp.content.read(256)
                preview = body.decode("utf-8", "ignore").replace("\n", " ")[:120]
                print(
                    f"[Yuki API] stream rejected: HTTP {resp.status}, "
                    f"content-type={content_type!r}, body={preview!r}"
                )
        except Exception as exc:
            print(
                f"[Yuki API] fast stream attempt={attempt} failed "
                f"for {video_id}: {type(exc).__name__}: {exc}"
            )
...
async def resolve(link_or_query: str, requested_by: str = "Unknown",
    ...
    # ULTRA-FAST PATH: Yuki direct stream is ALWAYS tried first.
    # Only a tiny Range request is used for validation; the bot never buffers
    # the complete media before playback. PyTgCalls receives the Yuki URL.
    metadata_duration = _seconds(info.get("duration"))
    media_path = await _yuki_fast_stream(info["id"], bool(video))

    # yt-dlp is strictly a secondary fallback and is never the normal path.
    if not media_path:
        print(
            f"[Playback] Yuki unavailable for {info['id']}; "
            f"using secondary downloader fallback"
        )
        media_path = await download_media(info["id"], is_video=bool(video))

    # Metadata duration avoids an extra ffprobe/network round-trip on the fast
    # path; local fallback still gets its accurate duration.
    resolved_duration = metadata_duration
    if not (str(media_path).startswith(("http://","https://"))) or not resolved_duration:
